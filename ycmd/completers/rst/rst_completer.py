...
LOGFILE_FORMAT = 'rst_http_{port}_{std}_'
PATH_TO_JEDIHTTP = os.path.abspath(
  os.path.join( os.path.dirname( __file__ ), '..', '..', '..',
                'third_party', 'rst_http', 'rst_http.py' ) )


class RstCompleter( Completer ):

  def __init__( self, user_options ):
    super( RstCompleter, self ).__init__( user_options )
    self._server_lock = threading.RLock()
    self._rst_http_port = None
    self._rst_http_phandle = None
  ...
  def ServerIsHealthy( self ):
    """
    Check if RstHTTP is alive AND ready to serve requests.
    """
    if not self._ServerIsRunning():
      self._logger.debug( 'RstHTTP not running.' )
      return False
    try:
  ...
  def _ServerIsRunning( self ):
    """
    Check if RstHTTP is alive. That doesn't necessarily mean it's ready to
    serve requests; that's checked by ServerIsHealthy.
    """
    with self._server_lock:
      return ( bool( self._rst_http_port ) and
               ProcessIsRunning( self._rst_http_phandle ) )


  def RestartServer( self, binary = None ):
    """ Restart the RstHTTP Server. """
    with self._server_lock:
      if binary:
    ...
    with self._server_lock:
      if self._ServerIsRunning():
        self._logger.info( 'Stopping JediHTTP server with PID {0}'.format(
                               self._rst_http_phandle.pid ) )
        self._rst_http_phandle.terminate()
        try:
          utils.WaitUntilProcessIsTerminated( self._rst_http_phandle,
                                              timeout = 5 )
          self._logger.info( 'RstHTTP server stopped' )
        except RuntimeError:
          self._logger.exception( 'Error while stopping RstHTTP server' )
